...
compiled_name = 'OleVbaAnalyzer'
python_file_name = 'main'

# Do not change anything
virtual_env_path = os.path.join(os.path.dirname(os.path.dirname(get_python_lib())), 'Scripts')
script_path = os.path.dirname(os.path.realpath(__file__))
reg_path = os.path.join(script_path, 'olevba_analyzer.reg')
bat_path = os.path.join(script_path, 'olevba_analyzer.cmd')
...
with open(reg_path, 'w') as f:
    f.write('Windows Registry Editor Version 5.00\n'
            '[HKEY_LOCAL_MACHINE\SOFTWARE\Classes\*\shell\Check for Malicious Macros]\n'
            '[HKEY_LOCAL_MACHINE\SOFTWARE\Classes\*\shell\Check for Malicious Macros\command]\n'
            '@="' + os.path.realpath(bat_path).replace('\\', '\\\\') + ' \\"%1\\""')

with open(bat_path, 'w') as f:
    f.write('cls\n"OleVbaAnalyzer.exe" -f %1\npause')
